#!/usr/bin/env python
import sys
sys.path.append('/home/curtis/DSA2')
...
import random, time, struct, math, time
...
#        return 8,900e6
        ...
        channel = int(random.choice([1,2]))
            

        hop_freq = float(1e9 * (2.408+(channel-1)*0.004))#setting the centre freq frequency for sending packets

        
        stats_array[channel] = stats_array[channel] + 1
        print "\nChannel DSA Selection Statistics (Channel #: Number times selected)"
        print "1: ", stats_array[1], " 2: ",stats_array[2]
    ...
    options_rx.rx_freq = 2.404e9 #setting default rx_freq value
